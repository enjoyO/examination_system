...
public class ExamineController {
	...
	public ModelAndView examine(String paperid){
		...
		//将本次测试加入测试表(如果是重复做同一套试卷的话，要将之前做这套试卷的信息覆盖)
	...
	public ModelAndView subSingle(String answer,String questionid){
		...
		Examine examinee=Examine.getInstance();
		//将答案提交到答题库,如果之前做过该题目要将答案跟新应该是问题和用户
		List<AnswerBean> alist=adi.queryPaperIdAndExamineeIdAndQuestionid(examinee.getPaperid(), examinee.getExamineeid(), questionid);
		if(alist.size()==0){
			//之前没有做过提交答案
		...
		if(number+1<all.size()){
			...
			all.clear();
			allhash.clear();
			mav.setViewName("redirect:/score?paperid="+examinee.getPaperid()+
